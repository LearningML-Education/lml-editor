...
export class MobilenetService {
  ...
  async load() {
    ...
    const url = this.MODEL_INFO[this.version][this.alpha].url;
    this.mobilenet = await tfconv.loadGraphModel(url, { fromTFHub: true });
    // Warmup the model.
    const result = tf.tidy(
      () => this.mobilenet.predict(tf.zeros(
        [1, this.IMAGE_SIZE, this.IMAGE_SIZE, 3])))
    ...
    return tf.tidy(() => {
      ...
      if (img.shape[0] !== this.IMAGE_SIZE || img.shape[1] !== this.IMAGE_SIZE) {
        ...
        resized = tf.image.resizeBilinear(
          normalized, [this.IMAGE_SIZE, this.IMAGE_SIZE], alignCorners);
      }
      ...
      if (embedding) {
        const embeddingName = this.EMBEDDING_NODES[this.version];
        const internal =
          this.mobilenet.execute(batched, embeddingName);
        result = tf.squeeze(internal, [1, 2]);
      } else {
        const logits1001 = this.mobilenet.predict(batched);
